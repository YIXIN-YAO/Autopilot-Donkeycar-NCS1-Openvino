...
class DriveMode:
    ...
    def run(self, mode, user_angle, user_throttle, pilot_angle, pilot_throttle):
        if mode == 'user':
            #user_throttle = user_throttle * 0.9
            #print(user_throttle)
            ...
            print(pilot_throttle * self.cfg.AI_THROTTLE_MULT)
...
def drive(cfg, angle_model=None, angle_model_type=None, th_model=None, th_model_type=None):
    """
    ...
    ie = IECore()
    # adding the auto-pilot throttle
    #th_model_type = 'categorical'
    th_model = None
    th_model = 'models/new_speed55.xml'
    if th_model_type is None:
        th_model_type = cfg.DEFAULT_MODEL_TYPE
    if th_model:
        kl2 = dk.utils.get_model_by_type(th_model_type, cfg)
        kl2.load(ie, model_path=th_model)
        inputs = ['cam/image_array']
        outputs = ['useless/angle', 'pilot/throttle']
        car.add(kl2, inputs=inputs, outputs=outputs, run_condition='run_pilot')

    # adding the auto-pilot angle
    if angle_model_type is None:
        angle_model_type = cfg.DEFAULT_MODEL_TYPE
    if angle_model:
        kl = dk.utils.get_model_by_type(angle_model_type, cfg)
        kl.load(ie, model_path=angle_model)
        inputs = ['cam/image_array']
        outputs = ['pilot/angle', 'useless/throttle']
...
if __name__ == '__main__':
    args = docopt(__doc__)
    cfg = dk.load_config()
    drive(cfg, angle_model=args['--model'], angle_model_type=args['--type'])
    # drive(cfg, angle_model=args['--angle_model'], angle_model_type=args['--angle_type'],
    #       th_model=args['--th_model'], th_model_type=args['--th_type'])
