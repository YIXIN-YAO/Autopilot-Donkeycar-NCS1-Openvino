"""
这个代码是2021.5.2.16.57h5topb文件夹里拷出来的，因为manage函数也在这个文件夹里。
...
class KerasPilot(ABC):
    ...
    def load(self, ie, model_path: str) -> None:
        #ie = IECore()
        model_xml = model_path
        model_bin = model_path[:-4] + '.bin'
        ...
        self.model = ie.load_network(network=self.net, num_requests=1, device_name='MYRIAD')
...
class KerasCategorical(KerasPilot):
    ...
    def inference(self, img_arr, other_arr):
        ...
        self.model.start_async(request_id=0, inputs={input_blob: img_arr})
        # 获取输出
        # angle = 0.0
        # throttle = 0.0
        if self.model.requests[0].wait(-1) == 0:
            output = self.model.requests[0].outputs

            throttle_binned = output['throttle_out/Softmax']
            angle_binned = output['angle_out/Softmax']
            N = len(throttle_binned[0])
            if N == 4:
                a = np.array([0.0, 0.225, 0.246, 0.246])  # 这个表就是红灯，黄灯，无灯，绿灯分别对应的速度，可以自己修改
                index = np.argmax(throttle_binned)
                throttle = a[index]
            else:  # angle模型会执行下面这个
...
def default_categorical(input_shape=(120, 160, 3)):
    ...
    # categorical output of throttle into 4 bins
    throttle_out = Dense(4, activation='softmax', name='throttle_out')(x)
